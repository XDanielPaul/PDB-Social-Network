...
from app.models.tag_model import PartialTagPostDTO, Tag, TagPost, TagPostDTO, TagReturn
...
class TagController(Controller):
    ...
    async def get_tags(self, db_session: AsyncSession) -> list[TagReturn]:
        ...
        with RabbitMQConnection() as conn:
            conn.publish_message('info', 'Server is getting all tags')
    ...
    async def create_tag(self, data: DTOData[TagPost], db_session: AsyncSession) -> TagReturn:
        ...
        check_if_exists = await db_session.execute(
            select(Tag).filter(Tag.name == add_this.get('name'))
        )
    ...
    async def delete_tag(self, id: UUID, db_session: AsyncSession) -> DeleteConfirm:
        ...
        await db_session.commit()
        return DeleteConfirm(deleted=True, message=f"Tag with id [{id}] was deleted.")
